...
class ApproveSignupRequestServiceTest :
    BehaviorSpec({
        isolationMode = IsolationMode.InstancePerLeaf

        val signupRequestRepository = mockk<NutritionistSignupRequestRepository>()
        val service = ApproveSignupRequestServiceImpl(signupRequestRepository)
        ...
        Given("PENDING 상태의 가입 요청이 있을 때") {
            ...
            When("승인하면") {
                ...
                Then("요청이 APPROVED가 되고 회원 역할이 NUTRITIONIST로 전환된다") {
                    response.status shouldBe SignupStatus.APPROVED
                    signupRequest.status shouldBe SignupStatus.APPROVED
                    pendingMember.role shouldBe Role.NUTRITIONIST
                }
            }
        }

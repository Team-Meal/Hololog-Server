...
class UpdateBudgetServiceTest :
    BehaviorSpec({
        ...
        val service = UpdateBudgetServiceImpl(budgetRepository, authenticationHolder)
        ...
        Given("부분 수정 요청이 주어졌을 때") {
            ...
            When("전달된 필드만 수정하면") {
                ...
                Then("null 필드는 유지하고 id를 반환한다") {
                    ...
                    budget.totalBudget shouldBe 1_000_000L
                    budget.usedBudget shouldBe 200_000L
            ...
            When("예산을 수정하면") {
                Then("InvalidBudgetException이 발생하고 저장하지 않는다") {
                    shouldThrow<InvalidBudgetException> {
                        service.execute(10L, UpdateBudgetRequest(usedAmount = 1_000_001L))
                    }
                    verify(exactly = 0) { budgetRepository.saveAndFlush(any()) }
                }
            }

...
class KamisPriceController(
    ...
    @PostMapping("/sync")
    fun syncPrices(): List<IngredientPriceResponse> = kamisPriceService.syncPrices()

    @Operation(summary = "가격 급등 품목 조회", description = "전일 대비 18% 이상 상승한 품목과 대체 추천 품목을 반환합니다.")
    @ApiResponses(ApiResponse(responseCode = "200", description = "조회 성공"))
    @GetMapping("/alerts")
    fun getPriceAlerts(): List<IngredientPriceResponse> = kamisPriceService.getPriceAlerts()
}
